...
class OldQuoteConfig(BaseModel):
    ...
    旧语录库配置，用于集成旧的语录数据源
    """
    enable: bool = Field(default=True, description="是否启用旧语录库功能")
    base_url: str = Field(default="http://artonelicfrequence.icu/", description="旧语录库 API 的基础 URL")
    timeout: int = Field(default=5, description="请求旧语录库 API 的超时时间（秒）")

class PermissionConfig(BaseModel):
    """
    权限配置，控制哪些群组或用户可以使用语录功能
    """
    quote_managers: List[int] = Field(default=[2435206827], description="拥有管理语录权限的用户 ID 列表")

    mode_group: str = Field(default="white", description="群权限模式，可选值为 'white' (白名单) 或 'black' (黑名单)")
    white_group_list: List[int] = Field(default=[980606481, 732909252], description="白名单，允许使用语录功能的群组 ID 列表")
...
class Config(BaseModel):
    ...
    old_quote_config: OldQuoteConfig = Field(default_factory=OldQuoteConfig, description="旧语录库配置")