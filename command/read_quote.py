...
async def f_quote_list(event: GroupME):
    ...
    # 精确匹配
    filt: Callable[[QuoteInfoV2], bool] = lambda quote: key == quote.author_name or key == quote.author_card or key == str(quote.author_id)
...
async def f_quote_search(event: GroupME):
    """
    语录搜索
    """
    key = event.get_plaintext().strip()
